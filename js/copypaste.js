...
(function(){
  ...
  var copyPasteGrid = Ext.extend( Ext.grid.GridPanel, {
    initComponent: function () {
      var config = {};
      this.buildConfig(config);
      Ext.apply(this, Ext.apply(this.initialConfig, config));
      copyPasteGrid.superclass.initComponent.call(this);
    },
    buildConfig: function (config) {
      this.buildStore(config);
      this.buildSelModel(config);
      this.buildColumns(config);
      this.buildFar(config);
    },
    buildStore: function (config) {
      config.store = new Ext.data.JSONStore({
        storeId: 'fakeDataStore',
        data: fakeData,
        root: 'items',
        idProperty: 'rank',
